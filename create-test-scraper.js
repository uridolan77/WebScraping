...
async function createTestScraper() {
  try {
    ...
    // Create a scraper status entry
    const statusQuery = `
      INSERT INTO scraper_status (scraper_id, is_running, has_errors, last_run, elapsed_time, pages_processed, pages_queued, current_depth, message, last_error)
      VALUES (?, 0, 0, NULL, '00:00:00', 0, 0, 0, 'Ready to run', '')
    `;
    
    const [statusResult] = await connection.execute(statusQuery, [scraperId]);
