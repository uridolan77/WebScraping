...
namespace WebScraper.RegulatoryContent
{
    ...
    public class GamblingRegulationMonitor
    {
        ...
        /// </summary>
        public Task<RegulatoryChangeResult?> MonitorForChanges(string url, string oldContent, string newContent)
        {
            try
            {
                ...
                return Task.FromResult<RegulatoryChangeResult?>(changeResult);
            }
            catch (Exception ex)
            {
                _logger($"Error monitoring changes for {url}: {ex.Message}");
                return Task.FromResult<RegulatoryChangeResult?>(null);
            }
        }
