...
using WebScraperApi.Services; // Add this to resolve IScraperService
    ...
    public class ScraperController : ControllerBase
    {
        ...
        public async Task<IActionResult> StartScraper(string id)
        {
            try
            {
                ...
                // Use the ScraperService to actually start the scraper
                var success = await _scraperService.StartScraperAsync(id);
                if (!success)
                {
                    return StatusCode(500, new { 
                        Message = $"Failed to start scraper {id}",
                        Error = "Scraper execution failed. Check logs for details."
                    });
                }

                // Get the updated status after starting
                var status = await _scraperRepository.GetScraperStatusAsync(id);
                if (status == null)
                {
                    // Create a status if none exists
        ...
        /// <summary>
        /// Get detailed monitoring information for a scraper
        /// </summary>
        /// <param name="id">The ID of the scraper to monitor</param>
        /// <returns>Detailed monitoring information for the scraper</returns>
        [HttpGet("{id}/monitor")]
        [ProducesResponseType(typeof(object), 200)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> MonitorScraper(string id)
        {
            try
            {
                var scraperConfig = await _scraperRepository.GetScraperByIdAsync(id);
                if (scraperConfig == null)
                {
                    _logger.LogWarning("Scraper with ID {ScraperId} not found during monitoring request", id);
                    return NotFound($"Scraper with ID {id} not found");
                }
                ...
                if (status == null)
                {
                    status = new ScraperStatusEntity
                    {
                        ...
                        UrlsProcessed = 0,
                        HasErrors = false,
                        Message = "Idle - No monitoring data available",
                        LastUpdate = DateTime.Now
                    };
                }

                // Collect detailed monitoring information
                var monitoringData = new
                {
                    Status = status,
                    Configuration = MapEntityToModel(scraperConfig),
                    Performance = new
                    {
                        CpuUsage = 0.0, // To be implemented with actual metrics
                        MemoryUsage = 0.0, // To be implemented with actual metrics
                        RequestsPerMinute = 0.0, // To be implemented with actual metrics
                        AverageResponseTime = 0.0 // To be implemented with actual metrics
                    },
                    ...
                    SystemInfo = new
                    {
                        ServerTime = DateTime.Now,
                        ServerTimeUtc = DateTime.UtcNow,
                        ApiVersion = "1.0.0" // Should be taken from assembly version in a production environment
                    }
                };
            ...
            {
                _logger.LogError(ex, "Error monitoring scraper with ID {ScraperId}: {ErrorMessage}", id, ex.Message);
                return StatusCode(500, new
                {
                    Message = $"An error occurred while monitoring scraper {id}",
                    Error = ex.Message,
                    InnerError = ex.InnerException?.Message,
                    StackTrace = ex.StackTrace
                });
        ...
        public async Task<IActionResult> StopScraper(string id)
        {
            try
            {
                ...
                // Use the ScraperService to actually stop the scraper
                var success = await _scraperService.StopScraperAsync(id);
                if (!success)
                {
                    return StatusCode(500, new {
                        Message = $"Failed to stop scraper {id}",
                        Error = "Scraper stop operation failed. Check logs for details."
                    });
                }
