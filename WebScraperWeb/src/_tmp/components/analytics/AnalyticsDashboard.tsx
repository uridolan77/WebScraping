...
const AnalyticsDashboard = () => {
  ...
  const [timeRange, setTimeRange] = useState<string>('week'); // 'day', 'week', 'month'
  const [popularDomains, setPopularDomains] = useState<PopularDomain[]>([]);
  const [changeFrequency, setChangeFrequency] = useState<ChangeFrequency | null>(null);
  const [scraperPerformance, setScraperPerformance] = useState<PerformanceData[]>([]);
  ...
  const getDateBySince = (range: string): Date => {
    const now = new Date();
    switch (range) {
      case 'day':
  ...
  const createSimulatedPerformanceData = (range: string) => {
