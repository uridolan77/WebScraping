...
import {
  ...
  Card, CardContent, Divider, SelectChangeEvent
} from '@mui/material';
import {
...
interface ContentChangeChartProps {
  data: ContentChangeData | null;
  isLoading: boolean;
  timeframe: string;
  onRefresh: () => Promise<any>;
}

const ContentChangeChart: React.FC<ContentChangeChartProps> = ({ data, isLoading, timeframe, onRefresh }) => {
  ...
  const calculateTrend = (data: any[], key: string): { value: number, direction: 'up' | 'down' | 'flat' } => {
  ...
  // Calculate totals
  const totals = chartData.reduce((acc: { added: number, modified: number, removed: number }, item: any) => {
