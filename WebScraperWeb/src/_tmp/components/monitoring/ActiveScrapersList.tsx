...
interface ActiveScrapersListProps {
  activeScrapers: Scraper[];
  isLoading: boolean;
  onStop: (id: string) => Promise<void>;
}

const ActiveScrapersList: React.FC<ActiveScrapersListProps> = ({ activeScrapers, isLoading, onStop }) => {
  ...
  // Format duration
  const formatDuration = (startTime: string | undefined): string => {
              ...
              <TableRow key={scraper.id} hover>
                ...
                <TableCell>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    {scraper.status?.urlsQueued && scraper.status.urlsQueued > 0 && (
