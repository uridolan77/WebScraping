...
const MonitoringDashboard: React.FC = () => {
  ...
  const [selectedScraper, setSelectedScraper] = useState<ScraperData | null>(null);
  ...
  const [expandedScrapers, setExpandedScrapers] = useState<Record<string, boolean>>({});
  ...
  const fetchScraperStatus = async (): Promise<void> => {
    setLoading(true);
    try {
  ...
  const fetchScraperLogs = async (scraperId: string) => {
  ...
  const handleStartScraper = async (scraperId: string) => {
    try {
      // In a real app, call your API endpoint
  ...
  const handleViewLogs = (scraper: ScraperData) => {
  ...
  const handleToggleExpand = (scraperId: string) => {
  ...
  const generateSimulatedScrapers = (): ScraperData[] => {
    return [
      {
        id: 'scraper-1',
        name: 'UKGC Monitor',
  ...
  const generateSimulatedLogs = (scraperId: string): ScraperLog[] => {
...
interface ScraperListProps {
  ...
  expandedScrapers: Record<string, boolean>;
  onToggleExpand: (scraperId: string) => void;
  onStart: (scraperId: string) => void;
  onStop: (scraperId: string) => void;
  onViewLogs: (scraper: ScraperData) => void;
  formatDuration: (startTime: string, endTime?: string | null) => string;
}

// Scraper List Component
const ScraperList: React.FC<ScraperListProps> = ({
  scrapers,
  expandedScrapers,
  onToggleExpand,
  onStart,
  ...
  const getStatusChip = (scraper: ScraperData) => {
