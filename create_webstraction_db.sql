-- SQL Script to create the webstraction_db database schema
-- For WebScraping project

-- Drop database if exists and create a new one
DROP DATABASE IF EXISTS webstraction_db;
CREATE DATABASE webstraction_db;
...
-- ScraperConfig table - stores configuration for scrapers
...
-- ScraperStartUrl table - stores additional start URLs for scrapers
CREATE TABLE scraper_start_url (
...
-- ContentExtractorSelector table - stores CSS selectors for content extraction
CREATE TABLE content_extractor_selector (
    id INT AUTO_INCREMENT PRIMARY KEY,
    scraper_id VARCHAR(36) NOT NULL,
...
-- KeywordAlert table - stores keywords to alert on
...
-- WebhookTrigger table - stores webhook trigger events
...
-- DomainRateLimit table - stores rate limits per domain
...
-- ProxyConfiguration table - stores proxy configurations
...
-- ScraperSchedule table - stores scheduling information
...
-- ScraperStatus table - stores current status of scrapers
...
-- ScraperRun table - stores individual scraper runs
...
-- PipelineMetric table - stores pipeline metrics
CREATE TABLE pipeline_metric (
    id INT AUTO_INCREMENT PRIMARY KEY,
    scraper_id VARCHAR(36) NOT NULL,
...
-- LogEntry table - stores log entries
...
-- ContentChangeRecord table - stores content changes
CREATE TABLE content_change_record (
    id INT AUTO_INCREMENT PRIMARY KEY,
    scraper_id VARCHAR(36) NOT NULL,
    url VARCHAR(500) NOT NULL,
...
-- ProcessedDocument table - stores processed documents
...
-- DocumentMetadata table - stores document metadata
...
-- ScraperMetrics table - stores scraper metrics
...
-- CustomMetric table - stores custom metrics
...
CREATE INDEX idx_scraper_run_scraper_id ON scraper_run(scraper_id);
CREATE INDEX idx_log_entry_scraper_id ON log_entry(scraper_id);
CREATE INDEX idx_content_change_record_scraper_id ON content_change_record(scraper_id);
CREATE INDEX idx_processed_document_scraper_id ON processed_document(scraper_id);
